...
export async function getRealtimeTrends(): Promise<Array<{ tag: string; posts: string; category: string }>> {
    try {
        const postsRef = collection(db, "posts")
        ...
        allPosts.forEach(p => {
            ...
            matches.forEach((match: string) => {
                ...
                // If it's a hashtag, preserve case, otherwise capitalize
        ...
        console.error("Error generating realtime trends", e)
        return [
            { tag: "VibezTube", posts: "24 posts", category: "News" },
            { tag: "Shorts", posts: "18 posts", category: "Entertainment" },
            { tag: "Live", posts: "12 posts", category: "Technology" },
